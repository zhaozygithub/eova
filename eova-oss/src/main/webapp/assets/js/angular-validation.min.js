            ...
            function($injector) {
                ...
                var validFunc = function(element, validMessage, validation, scope, ctrl) {
                    var successMsg = $validationProvider.getDefaultMsg(validation).success,
                        ...
                        validMessage = validMessage && validMessage.length ? validMessage.split('|') : '';
                    for (var i = 0; i < validArr.length; i++) {
                        var a = validArr[i].indexOf('='),
                            b = $.trim(a === -1 ? validArr[i] : validArr[i].substr(0, a)),
                            c = a === -1 ? element.attr(b) : validArr[i].substr(a + 1);
                        tempArr.push(b);
                        if (c) {
                            c = c.replace(new RegExp("\\[\|\\]", "g"), '').split('|');
                            for (var m = 0; m < c.length; m++) {
                                successMsg = successMsg.replace(new RegExp("\\{" + m + "\\}", "g"), function() {
                                    return c[m];
                                });
                            };
                        }
                    ...
                    var messageElem,
                        msgIndex = tempArr.indexOf(validation),
                        messageToShow = msgIndex !== -1 && validMessage[msgIndex] ? validMessage[msgIndex] : successMsg;

                    if (scope.messageId)
                        messageElem = angular.element(document.querySelector('#' + scope.messageId));
                    else
                        messageElem = element.siblings().last();
                ...
                var invalidFunc = function(element, validMessage, validation, scope, ctrl) {
                    var errorMsg = $validationProvider.getDefaultMsg(validation).error,
                        tempArr = [],
                        validArr = element.attr('validator').split(','),
                        validMessage = validMessage && validMessage.length ? validMessage.split('|') : '';
                    for (var i = 0; i < validArr.length; i++) {
                        ...
                        if (b == validation && c) {
                            c = c.replace(new RegExp("\\[\|\\]", "g"), '').split('|');
                            for (var m = 0; m < c.length; m++) {
                                errorMsg = errorMsg.replace(new RegExp("\\{" + m + "\\}", "g"), function() {
                                    return c[m];
                                });
                            };
                        }
                    ...
                    var messageElem,
                        msgIndex = tempArr.indexOf(validation),
                        messageToShow = msgIndex !== -1 && validMessage[msgIndex] ? validMessage[msgIndex] : errorMsg;
                ...
                var checkValidation = function(scope, element, attrs, ctrl, validation, value) {
                    ...
                    if (expression.constructor === Function) {
                        return $q.all([$validationProvider.getExpression(validator)(value, scope, element, attrs, validatorParam, $injector)])
                            .then(function(data) {
