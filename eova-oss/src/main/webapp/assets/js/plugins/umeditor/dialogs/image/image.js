...
//                }
    ...
    var Upload = {
        ...
        uploadComplete: function(r){
            ...
            try{
                 var start = r.indexOf(">");  
                 if(start != -1) {
                   var end = r.indexOf("<", start + 1);  
                   if(end != -1) {  
                     r = r.substring(start + 1, end);  
                    }  
                 }
                var json =eval('(' + r + ')');
                Base.callback(me.editor, me.dialog, json.data[me.editor.options.imageFieldName], json.status);
            }catch (e){
    ...
    UM.registerWidget('image', {
        ...
        width: 600,
        height: 300
    }, function (editor, $w, url, status) {
        Base.callback(editor, $w, url, status)
    })
})();
